...
# ── rembg: local background removal (PRIMARY segmentation path) ───────────────
try:
...
# BUGFIX: rembg's default model changed upstream to "bria-rmbg" — a ~1GB
# model that OOM-kills on memory-constrained hosts (Render free 512MB, a
# default-sized Cloud Run container, etc.) and adds a large one-time
# download on cold start. Pin to the classic "u2net" model (176MB, loads in
# a few seconds, comfortable in 512MB+ RAM) and cache the session so it
# builds once per process, not once per request.
_REMBG_SESSION = None
def _get_rembg_session():
    global _REMBG_SESSION
    if _REMBG_SESSION is None and _rembg_new_session is not None:
...
CORS_ALLOWED_ORIGINS: List[str] = [
    ...
    "https://www.luminorbit1.dpdns.org",
...
class _Settings:
    ...
    def _resolve_cloud_name(self) -> Optional[str]:
        # Primary: explicit env var — set CLOUDINARY_CLOUD_NAME in Railway
        direct = os.environ.get("CLOUDINARY_CLOUD_NAME")
        if direct and direct.strip():
            return direct.strip()

        # FIX: Railway dashboard shows variable as CLOUDFLARE_CLOUD_NAME (typo in Railway setup)
        # Reading both names so Cloudinary works regardless of which was set in Railway
        railway_alt = os.environ.get("CLOUDFLARE_CLOUD_NAME")
        if railway_alt and railway_alt.strip():
            return railway_alt.strip()

        # Fallback: some integrations encode cloud name in API key as "<cloudname>-<hex>"
...
PIPELINE_CAPABILITY: Dict[str, str] = {
    ...
    "denoising":       "denoising",
    "controlnet":      "image-enhancement",
    "color-matching":  "image-enhancement",
    "visualization":   "image-gen",
    "audio-sync":      "audio-extraction",
...
CAPABILITY_PROVIDERS: Dict[str, List[str]] = {
    ...
    "image-gen":         ["pollinations", "together", "krea", "segmind", "huggingface", "openrouter"],
    # ── Upscaling / super resolution (Segmind/Krea/HF all have working handlers) ──
    "super-resolution":  ["segmind", "krea", "huggingface"],
    # ── Background removal — rembg runs locally inside HuggingFaceAdapter ──────────
    "segmentation":      ["huggingface"],
    # ── Inpainting / object removal ───────────────────────────────────────────────
    "inpainting":        ["segmind", "huggingface", "deepai"],
    # ── Face processing / enhancement ─────────────────────────────────────────────
    "face-processing":   ["huggingface", "krea", "deepai"],
    # ── Photo restoration / deblur / old photo repair ─────────────────────────────
    "restoration":       ["huggingface", "krea", "deepai"],
    # ── Image enhancement / quality improvement ───────────────────────────────────
    "image-enhancement": ["segmind", "huggingface", "cloudinary"],
    # ── Style transfer / artistic filters ─────────────────────────────────────────
    "style-transfer":    ["huggingface", "segmind", "together", "pollinations"],
    # ── Denoising / noise reduction ───────────────────────────────────────────────
    # cloudflare added last: pure-PIL median filter, zero API key needed —
    # guarantees denoising always has a free fallback (BUGFIX, see adapter)
    "denoising":         ["segmind", "huggingface", "cloudinary", "cloudflare"],
    # ── ControlNet / guided generation ───────────────────────────────────────────
    "controlnet":        ["segmind", "huggingface"],
    # ── Video generation ──────────────────────────────────────────────────────────
    "video-gen":         ["pollinations", "together", "pexels"],
    # ── Video temporal processing — Cloudflare PIL handler ────────────────────────
    "temporal":          ["cloudflare"],
    # ── AI captioning / description ───────────────────────────────────────────────
    "captioning":        ["gemini", "groq", "mistral", "together"],
    # ── Audio extraction / sync — Cloudflare PIL handler ──────────────────────────
    "audio-extraction":  ["cloudflare"],
    "audio-sync":        ["cloudflare"],
    # ── Image compression / format conversion ─────────────────────────────────────
    "compression":       ["cloudinary", "cloudflare"],
    # ── Basic processing (resize, crop, flip, rotate, watermark, collage) ─────────
    "basic-processing":  ["cloudinary", "cloudflare"],
    # ── Color matching / grading ──────────────────────────────────────────────────
    "color-matching":    ["cloudflare", "cloudinary"],
    # ── Data visualization / chart / diagram generation ───────────────────────────
    "visualization":     ["pollinations", "gemini", "together"],
}
...
TOOL_PIPELINE_MAP: Dict[str, str] = {
    ...
    # BUGFIX: these 12 registered UI tools (photoTools catalog) had no
    # TOOL_PIPELINE_MAP entry at all — fell through to the "basic" default
    # (basic-processing), which is wrong for anything AI-driven here and
    # would silently give e.g. "AI Face Consistency Engine" a plain resize
    # instead of face processing.
    "CompressIt":                       "compression",
    "Blend Mode":                       "enhancement",
    "AI Texture Recovery":              "restoration",
    "AI Smart Exposure Balance":        "enhancement",
    "AI Shadow Detail Restorer":        "restoration",
    "AI Dynamic Range Expander":        "enhancement",
    "AI Face Consistency Engine":       "face_processing",
    "AI Pose & Sketch Composer":        "img2img",
    "AI Identity Preservation Engine":  "face_processing",
    "AI Cinematic Scene Builder":       "generation",
    "AI Style & Character Transformer": "style_transfer",
    "AI Prompt Director":               "generation",
    ...
    "Blur Tool":                        "basic",
    "BlurIt":                           "basic",
    "Background Blur Tool":             "enhancement",
    "Mosaic Tool":                      "enhancement",
    "Perspective Corrector":            "enhancement",
    "PNG Converter":                    "basic",
    "Collage Maker":                    "basic",
    "Photo Stitcher":                   "basic",
    "Image Splitter":                   "basic",

    # ── FIX X-1: 56 tools missing from TOOL_PIPELINE_MAP ────────────────────
    # These tools existed in the HTML TOOL_CAPABILITY_MAP but had no backend
    # pipeline entry, so they silently routed to 'basic' (CSS filter only).
    # Enhancement variants
    "Image Lab":                        "enhancement",
    "Photo Crafter":                    "enhancement",
    "Image Crafter":                    "enhancement",
    "Photo Mixer":                      "enhancement",
    "Photo Mashup":                     "enhancement",
    "Image Adjuster":                   "enhancement",
    "Image Transformer":                "enhancement",
    "Batch Editor":                     "enhancement",
    "Frequency Separation Tool":        "enhancement",
    "Edge Refinement Tool":             "enhancement",
    "Sky Enhancer":                     "enhancement",
    "Hue Shifter":                      "enhancement",
    "Hue Rotate":                       "enhancement",
    "Tint Tool":                        "enhancement",
    "Color Tint":                       "enhancement",
    "Channel Mixer Pro":                "enhancement",
    "Color Palette Generator":          "enhancement",
    "Selective Color Range Editor":     "enhancement",
    "Advanced Histogram Adjuster":      "enhancement",
    "Threshold":                        "enhancement",
    "Binarize":                         "enhancement",
    "Resize Pro":                       "enhancement",
    "Resize Guru":                      "enhancement",
    "Image Resizer Pro":                "enhancement",
    "Rotate 90":                        "enhancement",
    "Flip & Rotate":                    "enhancement",
    "Aspect Ratio Converter":           "enhancement",
    "Focus Tool":                       "enhancement",
    "Lens Blur":                        "enhancement",
    "Tilt-Shift":                       "enhancement",
    "Image Stitcher":                   "enhancement",
    "Layer Editor":                     "enhancement",
    "Image Overlay":                    "enhancement",
    "Frame Generator":                  "enhancement",
    "FrameIt":                          "enhancement",
    "Texturizer":                       "enhancement",
    "Watermark Maker":                  "enhancement",
    "Text Adder":                       "enhancement",
    "Meme Maker":                       "enhancement",
    "Passport Image Pro":               "enhancement",
    "Image Converter":                  "enhancement",
    "Photo Splitter":                   "enhancement",
    # Denoising
    "Denoiser":                         "denoising",
    # Style transfer variants
    "Film Grain":                       "style-transfer",
    "Faded Film":                       "style-transfer",
    "Film Burn Effect":                 "style-transfer",
    "Duotone":                          "style-transfer",
    "Dual Tone Grade":                  "style-transfer",
    "Gradient Map Tool":                "style-transfer",
    "Glitch Generator":                 "style-transfer",
    "Emboss":                           "style-transfer",
    # Video temporal variants
    "Clip Splitter Tool":               "temporal",
    "J-Cut Master":                     "temporal",
    "L-Cut Pro":                        "temporal",
    "Multi Clip Timeline Tool":         "temporal",
    "Split Screen Sync":                "color-matching",
    "Storyboard Sync":                  "color-matching",
}
...
# BUGFIX (critical, confirmed current as of 2026): api-inference.huggingface.co
# was fully decommissioned and now returns HTTP 410 Gone for every request —
# this broke EVERY HuggingFace-routed capability in this app (super-resolution,
# inpainting, face-processing, restoration, style-transfer, captioning,
# denoising, controlnet, image-enhancement, and the segmentation remote
# fallback), regardless of whether HF_API_KEY was valid. The replacement is
# HF's new unified Inference Providers router. Model-path shape is unchanged
# ("/models/{model}"); the pipeline-style path is restructured (task now
# comes after the model, not before it) — see the one call site that builds
# a pipeline URL below.
HF_API_BASE      = "https://router.huggingface.co/hf-inference/models"
HF_PIPELINE_BASE = "https://router.huggingface.co/hf-inference/models"
...
HF_MODELS: Dict[str, str] = {
    ...
    "image-enhancement":   "timbrooks/instruct-pix2pix",  # FIX B-6: use img2img not text-to-image
}
    ...
    def analyze(
        ...
        if not image_bytes:
            return cls.QualityReport(
                accepted=False, overall_score=0.0, entropy_score=0.0,
                size_score=0.0, dimension_ok=False, is_blank=False,
                is_corrupted=True, rejection_reason="Output is empty (0 bytes)",
            )

        # BUGFIX: a segmentation/cutout result with large transparent areas
        # compresses extremely well (transparent pixels behave like solid
        # color to PNG) — a small, correctly-processed cutout of a simple
        # subject (e.g. Sticker Maker on a logo) can legitimately land under
        # TINY_IMAGE_THRESHOLD and was being hard-rejected here as "corrupt"
        # before ever reaching the dimension/alpha checks below, which are
        # the actually-meaningful signal for this kind of output. Detect
        # real alpha transparency first and let those through to the
        # checks further down instead of gating on raw byte size alone.
        has_real_alpha = False
        if _PIL_AVAILABLE and len(image_bytes) >= 200:
            try:
                has_real_alpha = _PILImage.open(io.BytesIO(image_bytes)).mode in ("RGBA", "LA")
            except Exception:
                has_real_alpha = False

        if len(image_bytes) < cls.TINY_IMAGE_THRESHOLD and not has_real_alpha:
            size_score = len(image_bytes) / cls.TINY_IMAGE_THRESHOLD
            return cls.QualityReport(
                accepted=False, overall_score=size_score, entropy_score=0.0,
                size_score=size_score, dimension_ok=False, is_blank=False,
                is_corrupted=True,
                rejection_reason=f"Output too small ({len(image_bytes)} bytes)",
            )
...
class SegmentationRefiner:
    ...
    def refine_mask(
        ...
        if not _PIL_AVAILABLE:
            raise ProviderError(provider, "Pillow not available for mask refinement")
...
class SecurityGate:
    ...
    def validate_file_bytes(cls, data: bytes, declared_mime: str, provider: str) -> None:
        ...
        # Allow WebP even if declared as image/jpeg (common browser behavior)
        if data[:4] == b'RIFF' and data[8:12] == b'WEBP':
            return  # valid WebP — accept regardless of declared MIME
...
async def _upload_or_inline(
    ...
    # CLOUDINARY_UPLOAD_PRESET is optional for signed uploads.
    # Signed uploads (api_key + api_secret + signature) work without a preset.
    # Only unsigned uploads require a preset — we use signed uploads.
    try:
...
class HuggingFaceAdapter:
    ...
    CAPABILITIES = {
        ...
        "captioning", "image-enhancement", "denoising", "controlnet",
    }
    ...
    async def _call_segmentation(
        ...
        # ── PRIMARY: rembg local (zero latency, no API key needed) ─────────
        if _REMBG_AVAILABLE and _PIL_AVAILABLE:
            try:
                log.info("[HF-SEG] Trying rembg local (PRIMARY) bytes=%d", len(file_bytes))
                loop = asyncio.get_event_loop()
                # Run CPU-bound rembg in thread pool to avoid blocking event loop
                result_bytes: bytes = await loop.run_in_executor(
                    None, lambda: _rembg_remove(file_bytes, session=_get_rembg_session())
                )
                if result_bytes and len(result_bytes) > 1024:
                    # Validate it's a valid PNG with alpha
                    try:
                        img = _PILImage.open(io.BytesIO(result_bytes))
                        if img.mode == "RGBA":
                            out = io.BytesIO()
                            img.save(out, format="PNG", optimize=True)
                            final = out.getvalue()
                            log.info("[HF-SEG] rembg SUCCESS bytes=%d mode=%s", len(final), img.mode)
                            return {
                                "success": True,
                                "output": _bytes_to_data_uri(final, "image/png"),
                                "provider": cls.NAME,
                                "model": "rembg-local",
                            }
                        else:
                            ...
                            final = out.getvalue()
                            log.info("[HF-SEG] rembg SUCCESS (converted to RGBA) bytes=%d", len(final))
                            return {
                                "success": True,
                                "output": _bytes_to_data_uri(final, "image/png"),
                                "provider": cls.NAME,
                        ...
                        log.warning("[HF-SEG] rembg output PIL validation failed: %s — trying HF", pil_err)
                else:
                    log.warning("[HF-SEG] rembg returned tiny/empty result (%d bytes) — falling back to HF", len(result_bytes) if result_bytes else 0)
            except Exception as rembg_err:
                log.warning("[HF-SEG] rembg local failed: %s — falling back to HF", rembg_err)

        # ── SECONDARY: HuggingFace remote models ─────────────────────────────
        # rembg (above) is the only key-free path. If it didn't return, only
        # attempt the network fallback when we actually have a key to use —
        # otherwise every request would burn 2 retries on guaranteed 401s.
        if not api_key:
            raise ProviderError(cls.NAME, "rembg local segmentation failed/unavailable and HF_API_KEY not configured")
        last_err = "no models attempted"

        for (model, prefer_pipeline, use_json) in HF_SEGMENTATION_MODELS:
            url_models   = f"{HF_API_BASE}/{model}"
            # BUGFIX: new router nests task under the model path
            # ("/models/{model}/pipeline/{task}"), not the old
            # "/pipeline/{task}/{model}" shape.
            url_pipeline = f"{HF_PIPELINE_BASE}/{model}/pipeline/image-segmentation"
    ...
    async def call(
        ...
        # ── BUGFIX: segmentation must NOT require HF_API_KEY. ───────────────
        # rembg runs 100% locally inside _call_segmentation (zero external
        # calls, zero key needed) and is meant to be the primary path — the
        # old code raised on a missing key before rembg ever got a chance to
        # run, silently killing every background-removal tool whenever
        # HF_API_KEY wasn't set. Route segmentation here first, unconditionally.
        if capability == "segmentation":
            if not file_bytes:
                raise ProviderError(cls.NAME, "No image provided for segmentation")
            return await cls._call_segmentation(api_key or "", file_bytes, file_mime, log)
        ...
        if capability in ("style-transfer", "image-enhancement", "inpainting",
                          "face-processing", "restoration", "denoising", "controlnet"):
            if not file_bytes:
                raise ProviderError(cls.NAME, f"No image for {capability}")
            prompt = params.get("prompt", "enhance this image, high quality, professional result")

            # InstructPix2Pix: a real image-to-image model that edits the uploaded image
            # Unlike FLUX (text-to-image), this takes image + instruction → modified image
            IMG2IMG_MODEL = "timbrooks/instruct-pix2pix"
            model_url = f"{HF_API_BASE}/{IMG2IMG_MODEL}"
            headers["Content-Type"] = "application/json"
            img_b64 = base64.b64encode(file_bytes).decode()
            payload_body = {
                "inputs": prompt,
            ...
            for attempt in range(2):
                try:
                    async with httpx.AsyncClient(timeout=timeout) as client:
                        resp = await client.post(model_url, json=payload_body, headers=headers)
                    if resp.status_code == 503 and attempt == 0:
                        log.info("[HF] InstructPix2Pix loading (503) — waiting 20s")
                    ...
                    if attempt == 0:
                        ...
                        continue
                    raise ProviderError(cls.NAME, f"Timeout on {IMG2IMG_MODEL}")
                except Exception as req_err:
                    raise ProviderError(cls.NAME, f"Request error: {req_err}")

            if resp is None:
                raise ProviderError(cls.NAME, "No response received from InstructPix2Pix")
            if resp.status_code == 503:
                raise ProviderError(cls.NAME, "InstructPix2Pix still loading (503)", 503)
            if resp.status_code == 401:
                raise ProviderError(cls.NAME, "HF_API_KEY invalid (401)", 401)
            if resp.status_code != 200:
                raise ProviderError(cls.NAME,
                    f"HTTP {resp.status_code}: {resp.text[:300]}", resp.status_code)
            _validate_response_image(resp.content, cls.NAME)
            return {
                "success":  True,
                "output":   _bytes_to_data_uri(resp.content, "image/jpeg"),
                "provider": cls.NAME,
                "model":    IMG2IMG_MODEL,
            }
    ...
    async def call(
        ...
        if capability == "inpainting":
            ...
            return {"success": True, "output": f"data:image/jpeg;base64,{img_b64}", "provider": cls.NAME}
                ...
                # Denoising, style-transfer, controlnet: use ESRGAN (best Segmind img2img)
        if capability in ("denoising", "style-transfer", "controlnet"):
            if not file_bytes:
                raise ProviderError(cls.NAME, f"No image for {capability}")
            img_b64 = base64.b64encode(file_bytes).decode()
            payload = {
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                resp = await client.post(
                    f"{cls.BASE}/{cls.MODELS.get('image-enhancement','esrgan-v1-x2plus')}",
...
class TogetherAdapter:
    NAME = "together"
    CAPABILITIES = {"image-gen", "style-transfer", "captioning", "video-gen", "visualization"}
...
class GeminiAdapter:
    NAME = "gemini"
    CAPABILITIES = {"captioning", "visualization"}
    ...
    async def call(
        ...
        # BUGFIX: gemini-1.5-flash was fully shut down 9/24/2025 — dead
        # model, guaranteed 404. Current GA flash-tier model as of 2026.
...
class GroqAdapter:
    ...
    async def call(
        ...
        # BUGFIX: llava-v1.5-7b-4096-preview was deprecated by Groq back in
        # Sept 2024. Current vision-capable model on Groq's active lineup.
...
class MistralAdapter:
    ...
    async def call(
        ...
        # BUGFIX: pixtral-12b-2409 deprecated by Mistral 12/2/2025. Their
        # own docs name this the direct replacement (same tier, vision-capable).
...
class OpenRouterAdapter:
    ...
    # BUGFIX: was "black-forest-labs/FLUX-1-schnell", a paid model — user
    # requested free-tier-only. Confirmed free on OpenRouter's own pricing page.
    IMAGE_MODEL = "sourceful/riverflow-v2.5-fast:free"
    ...
    async def call(
        ...
        if capability == "captioning":
            # BUGFIX: "google/gemini-flash-1.5" is a stale model name and not
            # a free-tier model. "openrouter/free" is OpenRouter's own
            # meta-router that auto-selects an available free model with the
            # capability the request needs (here: image understanding) — more
            # robust than hardcoding one free model slug that can get
            # deprecated or rotated out from under this code.
        ...
        # BUGFIX: OpenRouter has no "/images/generations" endpoint (that's
        # the OpenAI DALL-E convention this code was copied from). OpenRouter's
        # real dedicated Image API, launched June 2026, is POST {BASE}/images,
        # and it returns base64 in data[0].b64_json — not a hosted .url like
        # the old code expected. Every OpenRouter image-gen call 404'd before.
        prompt = params.get("prompt", "a high quality image")
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.post(
                f"{cls.BASE}/images",
                json={"model": cls.IMAGE_MODEL, "prompt": prompt},
                headers=auth_headers,
            )
        if resp.status_code == 401:
            raise ProviderError(cls.NAME, "OPENROUTER_API_KEY invalid (401)", 401)
        ...
        data_out = (resp.json().get("data") or [{}])[0]
        b64_out = data_out.get("b64_json")
        if not b64_out:
            raise ProviderError(cls.NAME, "No image data in response")
        return {"success": True, "output": f"data:image/png;base64,{b64_out}", "provider": cls.NAME}
...
class CloudflareAdapter:
    ...
    CAPABILITIES = {
        ...
        "denoising",  # BUGFIX: PIL branch already implemented this; was never reachable
    }
    ...
    async def call(
        ...
        # ── BUGFIX: PIL-local capabilities need ZERO Cloudflare credentials. ─
        # These never touch the Cloudflare API — moved above the credential
        # gate so a missing/invalid CF_ACCOUNT_ID or CF_AI_TOKEN can no longer
        # block basic-processing/compression/color-matching/temporal/
        # audio-extraction/audio-sync/denoising. Previously the checks below
        # raised before this branch was ever reached, killing all of these
        # tools whenever Cloudflare wasn't configured.
        if capability in ("compression", "basic-processing", "color-matching",
                          "temporal", "audio-extraction", "audio-sync", "denoising"):
            if not file_bytes:
                raise ProviderError(cls.NAME, f"No file provided for {capability}")

            try:
                from PIL import Image as _PILImg, ImageFilter as _PILFlt, ImageEnhance as _PILEnh
                import io as _io

                img = _PILImg.open(_io.BytesIO(file_bytes)).convert("RGB")

                if capability == "compression":
                    out = _io.BytesIO()
                    img.save(out, format="JPEG", quality=params.get("quality", 82), optimize=True)
                    output_bytes = out.getvalue()
                    output_mime  = "image/jpeg"

                elif capability == "denoising":
                    denoised = img.filter(_PILFlt.MedianFilter(size=3))
                    out = _io.BytesIO()
                    denoised.save(out, format="JPEG", quality=92, optimize=True)
                    output_bytes = out.getvalue()
                    output_mime  = "image/jpeg"

                elif capability == "color-matching":
                    brightness = params.get("brightness", 1.0)
                    contrast   = params.get("contrast", 1.05)
                    saturation = params.get("saturation", 1.1)
                    img = _PILEnh.Brightness(img).enhance(brightness)
                    img = _PILEnh.Contrast(img).enhance(contrast)
                    img = _PILEnh.Color(img).enhance(saturation)
                    out = _io.BytesIO()
                    img.save(out, format="JPEG", quality=92)
                    output_bytes = out.getvalue()
                    output_mime  = "image/jpeg"

                elif capability in ("temporal", "audio-extraction", "audio-sync"):
                    # Video-frame processing — return enhanced still
                    sharpened = img.filter(_PILFlt.SHARPEN).filter(_PILFlt.SMOOTH)
                    out = _io.BytesIO()
                    sharpened.save(out, format="JPEG", quality=90)
                    output_bytes = out.getvalue()
                    output_mime  = "image/jpeg"
                    ...
                    if (width, height) != (img.width, img.height):
                        img = img.resize((int(width), int(height)), _PILImg.LANCZOS)
                ...
                return {
                    "success":  True,
                    "output":   _bytes_to_data_uri(output_bytes, output_mime),
                    "provider": cls.NAME,
                    "model":    f"pil-local-{capability}",
                }

            except Exception as e:
                raise ProviderError(cls.NAME, f"PIL processing failed for {capability}: {e}")

        # ── Everything past this point actually calls the Cloudflare Workers
        #    AI API and genuinely needs real credentials. ────────────────────
        ...
        raise ProviderError(cls.NAME, f"Capability '{capability}' not supported by Cloudflare adapter")
...
class CloudinaryAdapter:
    NAME = "cloudinary"
    CAPABILITIES = {"compression", "basic-processing", "image-enhancement", "color-matching", "denoising"}
    ...
    async def call(
        ...
        timestamp     = str(int(time.time()))
        # Signed upload — no upload preset required (works with api_key + api_secret)
        sign_params: Dict[str, str] = {"timestamp": timestamp}
        signature     = cls._compute_signature(sign_params, secret)
        ...
        # Apply Cloudinary transformation based on capability
        if not transform:
            if capability == "compression":
                quality = params.get("quality", 80)
                transform = f"q_{quality},f_auto"
            elif capability == "basic-processing":
                w = params.get("width", "")
                h = params.get("height", "")
                if w and h:
                    transform = f"c_fit,w_{int(w)},h_{int(h)}"
                elif w:
                    transform = f"c_scale,w_{int(w)}"
            elif capability == "image-enhancement":
                transform = "e_improve,e_auto_contrast"
            elif capability == "color-matching":
                transform = "e_improve:50,e_saturation:20"
            elif capability == "denoising":
                transform = "e_improve,e_sharpen:30"
...
class PipelineEngine:
    ...
    async def _run_inner(
        ...
        self._log.info("=" * 60)
        self._log.info("[pipeline] START tool=%s cap=%s req=%s file_bytes=%s mime=%s",
        ...
        IMAGE_CAPABILITIES = {
            ...
            "denoising", "style-transfer", "controlnet",
        }
        ...
        if not providers:
            ...
            if capability == "segmentation":
                return {
                    ...
                    "error_user_message": "Background removal is currently unavailable. Please try again shortly.",
                    "message":            "No segmentation providers configured or healthy (rembg/PIL missing on server, or provider circuit open).",
                    "tool":               tool,
        ...
        self._log.info("[pipeline] PROVIDER_CHAIN cap=%s chain=%s req=%s", capability, providers, request_id)
        ...
        for provider_name in providers:
            ...
            self._log.info("[pipeline] PROVIDER_ATTEMPT provider=%s cap=%s req=%s attempt=%d/%d",
            ...
            try:
                ...
                self._log.info("[pipeline] DELIVERY delivery=%s output_len=%s req=%s",
                    delivery,
                    f"{len(raw_output)}chars" if isinstance(raw_output, str) else "N/A",
                    request_id,
                )
                ...
                self._log.info("[pipeline] END tool=%s cap=%s provider=%s ms=%d req=%s SUCCESS",
                    tool, capability, provider_name, exec_ms, request_id)
                self._log.info("=" * 60)
        ...
        self._log.error("=" * 60)
        self._log.error(
            "[pipeline] ALL_PROVIDERS_FAILED tool=%s cap=%s attempted=%s last_err=%r last_status=%d req=%s ms=%d",
...
def _build_pipeline_engine_inner(
    ...
    for name, (ok, reason) in credential_checks.items():
        if ok:
            ...
            # BUGFIX (critical): this used to also do
            # `engine._health[name] = 0.0`, which made _sorted_providers()
            # filter the provider out of EVERY capability's candidate list —
            # including capabilities that don't need this credential at all
            # (huggingface/segmentation → rembg is local; cloudflare/
            # basic-processing+compression+color-matching+temporal+
            # audio-extraction+audio-sync+denoising → pure PIL, local).
            # A missing HF_API_KEY was silently disabling background removal
            # even though it needs no key, same for the Cloudflare PIL tools.
            # Health now degrades only from real call outcomes via
            # record_success()/record_failure() below — a provider that
            # genuinely needs a missing key still fails fast per-call (cheap,
            # ~0ms, raised before any network I/O) and the existing fallback
            # loop moves to the next provider in the chain, same as any
            # other runtime failure.

    log.info("[pipelines] ══ Luminorbit v37 Startup ══")
    log.info("[pipelines] Python rembg available: %s", _REMBG_AVAILABLE)
    log.info("[pipelines] Python Pillow available: %s", _PIL_AVAILABLE)
    log.info("[pipelines] Python NumPy available:  %s", _NUMPY_AVAILABLE)
    log.info("[pipelines] Tools registered:        %d", len(TOOL_PIPELINE_MAP))
    log.info("[pipelines] Pipelines registered:    %d", len(PIPELINE_CAPABILITY))
    log.info("[pipelines] Capability chains:       %d", len(CAPABILITY_PROVIDERS))
    ...
    if _REMBG_AVAILABLE:
        log.info("[pipelines] ✓ rembg available — LOCAL background removal active (PRIMARY segmentation path)")
    else:
        log.warning("[pipelines] ⚠ rembg NOT installed — segmentation falls through to HuggingFace API only")
        log.warning("[pipelines]   To enable local BG removal: pip install rembg[gpu] OR pip install rembg")

    seg_chain  = CAPABILITY_PROVIDERS["segmentation"]
    seg_active = [p for p in seg_chain if engine._health.get(p, 1.0) > 0.0]
    if not seg_active:
        log.error("[pipelines] ⚠ Segmentation provider circuit open (repeated failures) — check rembg/Pillow install and HF_API_KEY")
    else:
...
# ─────────────────────────────────────────────────────────────
# FIX B-10: /api/claude-proxy — server-side Anthropic proxy
# Prevents exposing API keys in the browser. Required for all
# AI Prompt tools (photo tools 76-100, video tools 76-100).
# Without this endpoint, executePrompt() gets 404 and falls
# back to the CSS filter path for ALL prompt-based tools.
# ─────────────────────────────────────────────────────────────
@app.post("/api/claude-proxy")
async def claude_proxy(request: Request) -> JSONResponse:
    """
    Prompt proxy for AI text tools in the frontend.
    Uses Gemini → Groq → Mistral fallback chain (no Anthropic key needed).
    Returns Anthropic-compatible response shape so index.html works unchanged.
    """
    try:
    ...
    # Extract the user prompt from the Anthropic-format messages body
    ...
    if not user_text:
        user_text = body.get("system", "Generate a professional high-quality image enhancement")

    max_tokens = min(int(body.get("max_tokens", 400)), 1000)

    def _anthropic_wrap(text: str) -> dict:
        """Wrap plain text into Anthropic-compatible response shape."""
        return {
    ...
    if gemini_key:
        try:
            # BUGFIX: gemini-1.5-flash was shut down 9/24/2025 — dead model.
            ...
            if resp.status_code == 200:
                data = resp.json()
                text_out = (data.get("candidates", [{}])[0]
                            .get("content", {}).get("parts", [{}])[0]
                            .get("text", ""))
                if text_out:
                    logger.info("[claude-proxy] Gemini success")
                    return JSONResponse(_anthropic_wrap(text_out))
        except Exception as e:
            logger.warning("[claude-proxy] Gemini failed: %s", e)
    ...
    if groq_key:
        try:
            groq_body = {
                # BUGFIX: llama3-8b-8192 (old naming, no version) is off
                # Groq's active lineup. Their own deprecation notice
                # recommends this as the direct replacement for 8B-tier.
            ...
            if resp.status_code == 200:
                ...
                if text_out:
                    logger.info("[claude-proxy] Groq success")
                    return JSONResponse(_anthropic_wrap(text_out))
        except Exception as e:
            logger.warning("[claude-proxy] Groq failed: %s", e)

    # ── Provider 3: Mistral ───────────────────────────────────────────────────
    mistral_key = _settings.MISTRAL_API_KEY
    if mistral_key:
        try:
            mistral_body = {
            ...
            if resp.status_code == 200:
                ...
                text_out = data.get("choices", [{}])[0].get("message", {}).get("content", "")
                if text_out:
                    logger.info("[claude-proxy] Mistral success")
                    return JSONResponse(_anthropic_wrap(text_out))
        except Exception as e:
            logger.warning("[claude-proxy] Mistral failed: %s", e)

    # ── Fallback: neutral CSS-compatible JSON ─────────────────────────────────
    logger.warning("[claude-proxy] All LLM providers failed — returning neutral fallback")
    return JSONResponse(_anthropic_wrap(
        '{"brightness":1.0,"contrast":1.05,"saturation":1.1,"hueRotate":0,'
        '"sepia":0,"blur":0,"description":"Enhancement applied"}'
    ))
...
async def health() -> JSONResponse:
    ...
    return JSONResponse({
        "status":       "ok",
        "version":      "v37",
        "service":      "luminorbit-backend",
        "providers":    stats,
        "capabilities": list(CAPABILITY_PROVIDERS.keys()),
        "tools_count":  len(TOOL_PIPELINE_MAP),
...
async def process(request: Request) -> JSONResponse:
    # ── Production rate limiting (60 req/min per IP) ─────────────────────────
    client_ip = request.client.host if request.client else "unknown"
    _rate_counters = getattr(app.state, "_rate_counters", {})
    import time as _time
    now_min = int(_time.time() // 60)
    key = f"{client_ip}:{now_min}"
    _rate_counters = {k: v for k, v in _rate_counters.items() if k.split(":")[1] == str(now_min)}
    _rate_counters[key] = _rate_counters.get(key, 0) + 1
    app.state._rate_counters = _rate_counters
    if _rate_counters[key] > 60:
        return JSONResponse({"success": False, "error_code": "RATE_LIMITED",
            "message": "Too many requests. Max 60/minute per IP."}, status_code=429)
    t_route = time.monotonic()
    request_id = str(uuid.uuid4())

    logger.info("=" * 80)
    logger.info("[PROCESS] REQUEST START req=%s method=POST path=/api/process", request_id)

    # ── 1. Parse body ────────────────────────────────────────────────────────
    try:
        raw_body = await request.body()
        if not raw_body:
            logger.error("[PROCESS] EMPTY BODY req=%s", request_id)
            return _error_response("EMPTY_BODY", "Request body is empty", status=400)
        body = json.loads(raw_body)
    except json.JSONDecodeError as je:
        logger.error("[PROCESS] JSON PARSE FAILED req=%s err=%s", request_id, je)
        return _error_response("INVALID_JSON", f"Request body is not valid JSON: {je}", status=400)
    except Exception as e:
        logger.exception("[PROCESS] BODY READ FAILED req=%s", request_id)
        return _error_response("BODY_READ_ERROR", f"Could not read request body: {e}", status=400)

    # ── 2. Extract fields — accept ALL frontend payload variants ──────────────
    tool       = body.get("tool", "")
    capability = body.get("capability", "basic-processing")
    # ── Server-side capability override (safety net) ─────────────────────────
    # If tool is known, derive the authoritative capability from TOOL_PIPELINE_MAP
    # + PIPELINE_CAPABILITY regardless of what the frontend sent.
    # This prevents misrouting if frontend sends a stale or wrong capability.
    if tool:
        _server_pipeline = TOOL_PIPELINE_MAP.get(tool)
        if _server_pipeline and _server_pipeline in PIPELINE_CAPABILITY:
            capability = PIPELINE_CAPABILITY[_server_pipeline]
    params     = body.get("params") or {}
    resolution = body.get("resolution", "1024x1024")
    user_id    = body.get("user_id", "anonymous")
    # Accept overriding request_id from body (useful for end-to-end tracing)
    request_id = body.get("request_id") or request_id

    # File field — accept all variants: file, file_data, image, image_data
    file_b64 = (
        body.get("file")        or
        body.get("file_data")   or
        body.get("image")       or
        body.get("image_data")  or
        ""
    )
    # MIME field — accept all variants: mime, file_mime, content_type
    file_mime = (
        body.get("mime")         or
    ...
    logger.info(
        "[PROCESS] PARSED tool=%r cap=%r mime=%r file=%s params_keys=%s req=%s",
        tool, capability, file_mime,
        f"{len(file_b64)} chars" if file_b64 else "NONE",
        list(params.keys())[:10],
        request_id,
    )
    ...
    # ── 3. Validate tool ────────────────────────────────────────────────────
    if not tool:
        logger.error("[PROCESS] MISSING_TOOL req=%s", request_id)
        return _error_response("MISSING_TOOL", "Required field 'tool' is missing", status=400)

    if not isinstance(params, dict):
        logger.warning("[PROCESS] params not dict — resetting to {} req=%s", request_id)
        params = {}

    # ── 4. File size guard + Decode file bytes ──────────────────────────────
    MAX_UPLOAD_BYTES = 20_971_520  # 20 MB hard ceiling (base64 is ~33% larger)
    if file_b64 and len(file_b64) > (MAX_UPLOAD_BYTES * 4 // 3 + 64):
        logger.error("[PROCESS] FILE TOO LARGE b64_len=%d req=%s", len(file_b64), request_id)
        return _error_response("FILE_TOO_LARGE",
            "File exceeds the 20 MB upload limit. Please compress or resize your file.", status=413)

    file_bytes: Optional[bytes] = None
    if file_b64:
        try:
            raw_b64 = file_b64.split(",", 1)[1] if "," in file_b64 else file_b64
            # Strip whitespace/newlines that can break b64decode
            raw_b64 = raw_b64.strip().replace("\n", "").replace("\r", "").replace(" ", "")
            file_bytes = base64.b64decode(raw_b64, validate=False)
            if len(file_bytes) == 0:
                logger.error("[PROCESS] FILE_EMPTY after decode req=%s", request_id)
                return _error_response("INVALID_FILE", "File decoded to 0 bytes", status=400)
            logger.info("[PROCESS] FILE decoded bytes=%d req=%s", len(file_bytes), request_id)
        except Exception as e:
            logger.error("[PROCESS] BASE64_DECODE_FAILED req=%s err=%s", request_id, e)
    ...
    try:
        result = await asyncio.wait_for(
            ...
            timeout=300.0,  # 5 min hard ceiling
        )
    except asyncio.TimeoutError:
        elapsed_ms = int((time.monotonic() - t_route) * 1000)
        logger.error("[PROCESS] PIPELINE TIMEOUT tool=%s req=%s ms=%d", tool, request_id, elapsed_ms)
        result = {
            "success": False, "error_code": "PIPELINE_TIMEOUT",
            "message": f"Processing timed out after {elapsed_ms}ms",
        }
    except Exception as e:
        elapsed_ms = int((time.monotonic() - t_route) * 1000)
        logger.exception("[PROCESS] PIPELINE EXCEPTION tool=%s req=%s ms=%d", tool, request_id, elapsed_ms)
        result = {
            "success": False, "error_code": "PIPELINE_CRASH",
            "message": f"Internal error: {type(e).__name__}: {str(e)[:200]}",
        }

    # ── 7. Guarantee all required response fields ────────────────────────────
    ...
    result.setdefault("latency_ms",          result.get("execution_ms", 0))
    result.setdefault("fallback_used",       False)
    result.setdefault("fallback_attempted",  False)
    result.setdefault("providers_attempted", [])
    result.setdefault("warnings",            [])
    result.setdefault("request_id",          request_id)
    result.setdefault("mime",                file_mime)

    # Ensure output_url always populated when output exists
    if result.get("output") and not result.get("output_url"):
        result["output_url"] = result["output"]
    if result.get("output_url") and not result.get("preview_url"):
        result["preview_url"] = result["output_url"]

    total_ms = int((time.monotonic() - t_route) * 1000)
    logger.info(
        "[PROCESS] REQUEST END req=%s tool=%s success=%s provider=%s ms=%d delivery=%s",
        request_id, tool, result.get("success"), result.get("provider"),
        total_ms, result.get("delivery"),
    )
    logger.info("=" * 80)
