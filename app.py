...
CAPABILITY_PROVIDERS: Dict[str, List[str]] = {
    ...
    "denoising":         ["segmind", "huggingface", "cloudinary"],
        ...
        if _REMBG_AVAILABLE and _PIL_AVAILABLE:
            try:
                ...
                result_bytes: bytes = await loop.run_in_executor(
                    None, lambda: _rembg_remove(file_bytes)
                )
...
class CloudflareAdapter:
    ...
    async def call(
        ...
        # ── PIL-based local processing for capabilities where CF has no specific AI model ──
        # These capabilities use Cloudflare for delivery + PIL for actual processing
        if capability in ("compression", "basic-processing", "color-matching",
                          "temporal", "audio-extraction", "audio-sync"):
            if not file_bytes:
