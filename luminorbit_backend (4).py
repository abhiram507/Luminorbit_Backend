...
const APP_VERSION = "23.0.0-cf-4k";
...
const VALID_TOOLS = {
  ...
  // Photo tools
  "Background Remover":"segmentation","Background Changer":"inpainting",
  "Object Remover":"inpainting","Object Remover Pro":"inpainting",
  "Watermark Remover":"inpainting","Photo Restorer":"restoration",
  "Image Enhancer":"image-enhancement","Image Enhancer Plus":"image-enhancement",
  "HDR Master":"image-enhancement","Noise Reducer":"denoising",
  "Face Retouch":"face-processing","Portrait Pro":"face-processing",
  "Style Transfer":"style-transfer","Cartoonizer":"style-transfer",
  "Sky Replacer":"inpainting","Image Cropper":"basic-processing",
  "Photo Resizer":"basic-processing","Image UpScaler":"super-resolution",
  "Smart Crop":"segmentation","Color Corrector":"image-enhancement",
  "Color Grader":"image-enhancement","Sketch Maker":"style-transfer",
  "Lens Distortion Fix":"basic-processing","Black & White":"basic-processing",
  "Sepia Filter":"basic-processing","Vintage Maker":"style-transfer",
  "Vignette Tool":"basic-processing","Sharpen Tool":"image-enhancement",
  "Blur Tool":"basic-processing","Mosaic Tool":"basic-processing",
  "Exposure Fixer":"image-enhancement","Shadow Fixer":"image-enhancement",
  "Lighting Fixer":"image-enhancement","Detail Enhancer":"image-enhancement",
  // Video tools (additional)
  "Video Trimmer Pro":"temporal","Video Crop Studio":"basic-processing",
  "Color Grade Pro":"image-enhancement","Slow-Mo Magic":"temporal",
  "Fast-Forward Flash":"temporal","Cinematic Pulse":"video-gen",
  "VHS Nostalgia":"style-transfer","Neon Pulse":"style-transfer",
  "Glitch Pop":"style-transfer","Retro Reel":"style-transfer",
  "Motion Blur Trail":"basic-processing","Auto Caption Generator":"captioning",
  "Photo to Video Creator":"video-gen","AI Motion Transfer Engine":"temporal",
  "AI Cinematic Action Generator":"video-gen","AI Consistent Motion Animator":"temporal",
};
...
const TOOL_PROVIDERS = {
  ...
  "SDXL 1.0":             ["huggingface","pixazo","deepai","segmind"],
...
const CAPABILITY_PROVIDERS = {
  ...
  "super-resolution":  ["huggingface","cloudflare","krea","pixazo"],
  "image-enhancement": ["huggingface","segmind","pixazo","cloudflare"],
  "denoising":         ["huggingface","cloudflare"],
  "restoration":       ["huggingface","deepai","krea","pixazo"],
  "style-transfer":    ["huggingface","pollinations","together"],
  "captioning":        ["gemini","groq","mistral"],
  "audio-extraction":  ["cloudflare"],
  "compression":       ["cloudflare","cloudinary"],
  ...
  "image-gen":         ["pollinations","together","krea","segmind","pixazo"],
...
const PROVIDER_TIMEOUT_MS = 28000; // CF Workers limit ~30s CPU
const TARGET_W = 3840;
const TARGET_H = 2160;
...
function verifyAuth(request, env) {
  const secret = env.API_SECRET || "luminorbit_secure_123";
...
function getAllowedOrigins(env) {
  ...
  if (!raw || raw.trim() === "*") {
    // Default to known Luminorbit frontend domains when secret not set
    return [
      "https://luminorbit1.dpdns.org",
      "https://www.luminorbit1.dpdns.org",
      "https://luminorbit-1.pages.dev",
      "https://abhiram507.github.io",
      "https://api.luminorbit1.dpdns.org",
    ];
  }
...
async function handleHealth(env) {
  ...
  const apiStatus = {
    ...
    segmind: !!(env.SEGMIND_API_KEY),
    krea: !!(env.KREA_API_KEY),
    deepai: !!(env.DEEPAI_API_KEY),
    cloudflare_ai: !!(env.CF_AI_TOKEN && env.CF_ACCOUNT_ID),
    pexels: !!(env.PEXELS_API_KEY),
    unsplash: !!(env.UNSPLASH_API_KEY),
    pixazo: !!(env.PIXAZO_API_KEY),
    cloudinary: !!(env.CLOUDINARY_CLOUD_ID),
  };
  ...
  return jsonResp({
    ...
    quality: "4K Ultra HD",
    api_providers_configured: `${configuredCount}/15`,
    api_status: apiStatus,
    tools_supported: Object.keys(VALID_TOOLS).length,
    note: "4K studio quality output. Job state via KV.",
  });
}
...
async function callProvider(name, capability, fileBytes, fileMime, params, env) {
  switch (name) {
    ...
    case "pixazo":        return callPixazo(capability, fileBytes, fileMime, params, env);
    default: return { success: false, error: `unknown_provider:${name}` };
  }
}

// ── Tool → Rich Prompt Map (used when params.prompt is empty) ────────────────
function _buildToolPrompt(toolName, params) {
  const base = {
    "Background Remover":     "Remove background completely, isolate subject with pixel-perfect edges, transparent PNG output",
    "Background Changer":     "Replace background with professional studio backdrop, cinematic lighting, 4K",
    "Object Remover":         "Remove object seamlessly using AI inpainting, reconstruct background, no artifacts, 4K",
    ...
    "Detail Enhancer":        "Ultra-sharp detail enhancement: reveal micro-textures, professional sharpening, clarity boost",
    "Image UpScaler":         "AI super-resolution 4K upscaling: quadruple resolution, add realistic details, ultra-sharp",
    "AI 4K Image Upscaler":   "Real-ESRGAN 4K super resolution: maximum quality upscaling to 3840x2160",
    "Photo Restorer":         "Old photo restoration: remove damage, colorize, enhance to modern quality",
    "Lighting Fixer":         "Professional lighting correction: fix flat lighting, add dimension, studio quality illumination",
    "Shadow Fixer":           "Lift shadows and recover detail, balanced exposure, professional post-processing",
    "Exposure Fixer":         "Correct exposure perfectly: histogram equalization, recover highlights, lift shadows",
    ...
    "Sketch Maker":           "Pencil sketch effect: detailed line art, realistic pencil texture, black and white",
    "Vintage Maker":          "Vintage film effect: faded colors, film grain, light leaks, authentic analog look",
    "Color Grader":           "Professional cinematic color grade: teal-orange LUT, film emulation, Hollywood look",
    "Sharpen Tool":           "Professional sharpening: enhance edges, reveal fine detail, crisp studio quality",
    "Blur Tool":              "Selective blur: smooth background, sharp subject, depth-of-field effect",
    "Slow-Mo Magic":          "AI frame interpolation: ultra-smooth slow motion, 4K temporal super-resolution",
    "Fast-Forward Flash":     "Fast-forward video effect: smooth acceleration, no judder, professional output",
    "Cinematic Pulse":        "Cinematic color grade: film emulation, anamorphic lens flare, Hollywood grade",
    "VHS Nostalgia":          "VHS tape effect: scan lines, color bleeding, retro 80s video aesthetic",
    "Neon Pulse":             "Neon glow effect: vibrant colors, cyber aesthetic, professional grade",
    "Glitch Pop":             "Digital glitch effect: RGB split, scan lines, digital distortion art",
    "Retro Reel":             "Retro film effect: grain, color shifts, vintage film aesthetic",
    "Color Grade Pro":        "Professional video color grade: cinematic LUTs, teal-orange look, 4K",
    "Auto Caption Generator": "Generate accurate captions with speaker detection, professional formatting",
    "Photo to Video Creator": "Animate photo with natural motion: parallax effect, cinematic pan, 4K",
    "AI Motion Transfer Engine": "Transfer motion from source to target video, temporal consistency, 4K",
    "AI Cinematic Action Generator": "Generate cinematic action sequence with dramatic camera work, 4K",
    "AI Consistent Motion Animator": "AI animation with frame consistency, smooth temporal transitions, 4K",
  };
  const prompt = base[toolName] || ("Professional " + toolName + " processing: ultra high quality, 4K studio output, photorealistic, professional result");
  return prompt;
}
...
async function callPollinations(capability, fileBytes, fileMime, params, env) {
  ...
  const prompt = params.prompt || _buildToolPrompt("Flux 1.1 Pro", params) || "professional studio quality photograph ultra high detail 4K";
  const model  = (capability === "style-transfer" || capability === "restoration") ? "flux-pro" : "flux";
  const seed   = params.seed || 42;
  const url    = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=${TARGET_W}&height=${TARGET_H}&model=${model}&seed=${seed}&nologo=true&enhance=true&quality=100&upscale=true`;
...
// ── Pixazo ────────────────────────────────────────────────────────────────────
async function callPixazo(capability, fileBytes, fileMime, params, env) {
  const key = env.PIXAZO_API_KEY || "";
  if (!key) return { success: false, error: "pixazo:no_key" };
  const prompt = params.prompt || "ultra detailed professional studio photograph 4K high resolution";
  const body = {
    prompt, negative_prompt: "blurry, low quality, watermark, nsfw",
    model: "sdxl", width: TARGET_W, height: TARGET_H,
    steps: params.steps || 30, guidance_scale: params.cfg || 7.5, samples: 1,
  };
  if (fileBytes) { body.init_image = arrayBufferToBase64(fileBytes); body.strength = params.strength || 0.75; }
  const r = await fetch("https://api.pixazo.ai/v1/generate", {
    method: "POST",
    headers: { "Authorization": `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (r.status === 429) return { success: false, error: "pixazo:rate_limited" };
  if (!r.ok) return { success: false, error: `pixazo:${r.status}` };
  const data = await r.json();
  const b64 = data.image || data.output || data.data || "";
  if (!b64) return { success: false, error: "pixazo:no_image" };
  return { success: true, output: `data:image/png;base64,${b64}`, resolution: `${TARGET_W}x${TARGET_H}` };
}
...
_WRANGLER_TOML = """\
# wrangler.toml — Cloudflare Workers deployment config for Luminorbit
#
# SETUP:
#   1. npm install -g wrangler
#   2. wrangler login
#   3. wrangler kv:namespace create LUMINORBIT_JOBS
#      → paste the resulting id into [[kv_namespaces]] id field below
#   4. Set secrets (one command each):
#      wrangler secret put API_SECRET           → luminorbit_secure_123
#      wrangler secret put ALLOWED_ORIGINS      → https://luminorbit1.dpdns.org,https://luminorbit-1.pages.dev
...
#   5. wrangler deploy
#
# After deploy, your Worker URL is:
#   https://luminorbit.<YOUR_SUBDOMAIN>.workers.dev
# Or custom domain (from Cloudflare dashboard → Workers → Custom Domains)

name = "luminorbit-proxy"
main = "index.js"
compatibility_date = "2026-04-28"
account_id = "84fc2cd2ac091f95e12ae1960c601fe8"

# KV Namespace for async job state
  ...
  - type: web
    ...
    envVars:
      ...
      - key: WORKERS
        value: 1
      - key: API_SECRET
        value: luminorbit_secure_123
      - key: ALLOWED_ORIGINS
        value: "https://luminorbit1.dpdns.org,https://www.luminorbit1.dpdns.org,https://luminorbit-1.pages.dev,https://abhiram507.github.io"
      - key: CLOUDINARY_UPLOAD_PRESET
        value: luminorbit_signed
      - key: REDIS_URL
      ...
      - key: DAILY_REQUEST_LIMIT
        value: "200"
      # Provider API Keys (set these in Render.com dashboard as secrets)
      ...
      - key: CF_AI_TOKEN
        sync: false
      - key: CF_ACCOUNT_ID
        value: "84fc2cd2ac091f95e12ae1960c601fe8"
      - key: PEXELS_API_KEY
        sync: false
...
def _run_setup():
    ...
    print(f"  ✓ wrote {wjs} (Cloudflare Worker index.js)")
    print()
    ...
    print()
    print("Your deployment stack:")
    print("  CF Worker: https://luminorbit-proxy.luminorbit01.workers.dev")
    print("  Render API: https://luminorbit-backend.onrender.com")
    print("  Domain: https://luminorbit1.dpdns.org")
    print("  API domain: https://api.luminorbit1.dpdns.org")
    print()
...
class Settings(BaseSettings):
    ...
    API_SECRET: str = "luminorbit_secure_123"
    PORT:       int = 8000
    WORKERS:    int = 1
    ...
    ALLOWED_ORIGINS: str = "https://luminorbit1.dpdns.org,https://www.luminorbit1.dpdns.org,https://luminorbit-1.pages.dev,https://abhiram507.github.io,https://api.luminorbit1.dpdns.org,https://luminorbit-backend.onrender.com,https://luminorbit.pages.dev,https://luminorbit-proxy.luminorbit01.workers.dev,http://localhost:3000,http://localhost:5173,http://localhost:8080"
    ...
    def allowed_origins_list(self) -> List[str]:
        ...
        if not origins:
            # Fallback to known Luminorbit frontend domains (ALLOWED_ORIGINS default always set above)
            return [
                "https://luminorbit1.dpdns.org",
                "https://www.luminorbit1.dpdns.org",
                "https://luminorbit-1.pages.dev",
                "https://abhiram507.github.io",
                "https://api.luminorbit1.dpdns.org",
                "https://luminorbit-proxy.luminorbit01.workers.dev",
            ]
        return origins
    ...
    def validate_production(self) -> List[str]:
        ...
        if self.APP_ENV == "production":
            ...
            # Note: "luminorbit_signed" is a valid cloud name/preset — don't flag it
            if self.CLOUDINARY_CLOUD_ID.lower() in ("placeholder", "your-cloud-name", "example"):
                errors.append(
                    "CLOUDINARY_CLOUD_ID is still set to a placeholder. "
                    "Set it to your actual Cloudinary cloud name, or leave it empty to use data-URL fallback."
                )
            # ALLOWED_ORIGINS: warn but don't fatal-exit — Render may inject env vars after first boot
            if not self.ALLOWED_ORIGINS or self.ALLOWED_ORIGINS.strip() == "*":
                # Log as warning only — allow startup so the service comes online
                logger.warning(
                    "[startup] ALLOWED_ORIGINS not set or is wildcard '*'. "
                    "Set ALLOWED_ORIGINS=https://yourusername.github.io for strict CORS in production."
                )
    ...
    def log_startup(self):
        key_map = {
            ...
            "PIXAZO_API_KEY":       self.PIXAZO_API_KEY,
        }
...
VALID_TOOLS: Dict[str, str] = {
    ...
    "Beat Sync Drop": "audio-sync", "Sound Wave Viz": "visualization",
    "Audio Reactive Viz": "visualization",
    # ── Photo background / object ───────────────────────────────────────
    "Background Remover": "segmentation", "Background Changer": "inpainting",
    "Object Remover": "inpainting", "Object Remover Pro": "inpainting",
    "Watermark Remover": "inpainting", "Sky Replacer": "inpainting",
    "Transparent Background": "segmentation", "Smart Crop": "segmentation",
    # ── Photo restoration / enhancement ────────────────────────────────
    "Photo Restorer": "restoration", "Image Enhancer": "image-enhancement",
    "Image Enhancer Plus": "image-enhancement", "HDR Master": "image-enhancement",
    "HDR Booster": "image-enhancement", "Noise Reducer": "denoising",
    "Detail Enhancer": "image-enhancement", "Sharpen Tool": "image-enhancement",
    "Exposure Fixer": "image-enhancement", "Shadow Fixer": "image-enhancement",
    "Lighting Fixer": "image-enhancement",
    # ── Face / portrait ─────────────────────────────────────────────────
    "Face Retouch": "face-processing", "Portrait Pro": "face-processing",
    # ── Upscaling ───────────────────────────────────────────────────────
    "Image UpScaler": "super-resolution", "AI 4K Image Upscaler": "super-resolution",
    # ── Style transfer / artistic ───────────────────────────────────────
    "Style Transfer": "style-transfer", "Cartoonizer": "style-transfer",
    "Sketch Maker": "style-transfer", "Vintage Maker": "style-transfer",
    "Sepia Filter": "style-transfer", "Film Grain": "style-transfer",
    "Faded Film": "style-transfer", "Film Burn Effect": "style-transfer",
    "Duotone": "style-transfer", "Dual Tone Grade": "style-transfer",
    "Glitch Generator": "style-transfer", "Glitch Pop": "style-transfer",
    "VHS Nostalgia": "style-transfer", "Neon Pulse": "style-transfer",
    "Retro Reel": "style-transfer",
    # ── Color tools ─────────────────────────────────────────────────────
    "Color Corrector": "image-enhancement", "Color Grader": "image-enhancement",
    "Color Grade Pro": "image-enhancement", "Hue Shifter": "image-enhancement",
    "Saturation Booster": "image-enhancement", "Vibrance Tool": "image-enhancement",
    "Color Temperature": "image-enhancement", "White Balance": "image-enhancement",
    "Channel Mixer Pro": "image-enhancement",
    "Selective Color Range Editor": "image-enhancement",
    "Black & White": "image-enhancement", "Grayscale Tool": "image-enhancement",
    "B&W Converter": "image-enhancement", "Invert Colors": "image-enhancement",
    # ── Geometry / resize ───────────────────────────────────────────────
    "Image Cropper": "basic-processing", "Photo Resizer": "basic-processing",
    "Lens Distortion Fix": "basic-processing", "Lens Distortion Fixer": "basic-processing",
    "Vignette Tool": "basic-processing", "Blur Tool": "basic-processing",
    "Mosaic Tool": "basic-processing", "Image Rotator": "basic-processing",
    "Horizontal Flip": "basic-processing", "Vertical Flip": "basic-processing",
    "Mirror Effect": "basic-processing", "Perspective Corrector": "basic-processing",
    # ── Composite / overlay ─────────────────────────────────────────────
    "Photo Stitcher": "image-enhancement", "Image Stitcher": "image-enhancement",
    "Layer Editor": "image-enhancement", "Frame Generator": "image-enhancement",
    "Collage Maker": "image-enhancement", "Texturizer": "image-enhancement",
    "Watermark Maker": "image-enhancement", "Image Compressor Pro": "compression",
    # ── AI image generation (frontend names) ───────────────────────────
    "AI Image Generator": "image-gen", "AI Photo Creator": "image-gen",
    "AI Art Generator": "image-gen",
    "AI Ultra Fast Image Generator": "image-gen",
    "AI Cinematic Action Generator": "video-gen",
    "AI Environment & Scene Generator": "image-gen",
    # ── Video tools ─────────────────────────────────────────────────────
    "Slow-Mo Magic": "temporal", "Fast-Forward Flash": "temporal",
    "Cinematic Pulse": "video-gen", "Motion Blur Trail": "basic-processing",
    "Photo to Video Creator": "video-gen", "Video Trimmer Pro": "temporal",
    "Video Crop Studio": "basic-processing",
    "AI Motion Transfer Engine": "temporal",
    "AI Consistent Motion Animator": "temporal",
}
...
def validate_tool_capability(tool: str, capability: str) -> Tuple[str, str]:
    ...
    # Always resolve capability from VALID_TOOLS when tool is known — ignores frontend hint
    if tool in VALID_TOOLS:
        capability = VALID_TOOLS[tool]
    elif capability not in VALID_CAPABILITIES:
        capability = "basic-processing"
...
def validate_process_contract(data: Dict[str, Any]) -> List[str]:
    violations = []
    # tool must be present and non-empty
    if not data.get("tool"):
        violations.append("missing_required:tool")
    # capability must be present but we accept 'basic-processing' as universal fallback
    cap = data.get("capability", "")
    if cap and cap not in VALID_CAPABILITIES:
        # Silently remap unknown capability to basic-processing instead of hard-failing
        # This handles legacy/frontend clients sending stale capability values
        data["capability"] = "basic-processing"
    elif not cap:
        data["capability"] = "basic-processing"
    return violations
...
class BaseProvider(abc.ABC):
    ...
    def __init__(self, settings: Settings):
        ...
        self._lock         = asyncio.Lock()
        # Auto-disable if no API key (checked per-subclass)
        self.enabled       = True
    ...
    async def safe_process(self, capability: str, file_bytes: Optional[bytes],
        ...
        try:
            result = await asyncio.wait_for(
                ...
                timeout=max(self.settings.PROVIDER_TIMEOUT, 45),
            )
...
class PollinationsProvider(BaseProvider):
    ...
    def __init__(self, s: Settings):
        ...
        # Pollinations works without an API key — always enable as primary free provider
        self.enabled = True
...
class CloudflareProvider(BaseProvider):
    ...
    def __init__(self, s: Settings):
        ...
        self.enabled = bool(self.token and self.acct)
...
class CloudinaryProvider(BaseProvider):
    ...
    def __init__(self, s: Settings):
        ...
        self.enabled = bool(self.cloud and not self.cloud.isdigit())
...
TOOL_PROVIDERS: Dict[str, List[str]] = {
    ...
    # ── Photo editing tools (routed by capability) ──────────────────
    "Background Remover":           ["huggingface", "segmind", "cloudflare"],
    "Background Changer":           ["huggingface", "segmind", "deepai"],
    "Object Remover":               ["huggingface", "segmind", "deepai"],
    "Object Remover Pro":           ["huggingface", "segmind", "krea"],
    "Watermark Remover":            ["huggingface", "deepai", "segmind"],
    "Photo Restorer":               ["huggingface", "deepai", "krea"],
    "Sky Replacer":                 ["huggingface", "segmind", "pollinations"],
    "Face Retouch":                 ["huggingface", "deepai", "krea"],
    "Portrait Pro":                 ["huggingface", "krea", "deepai"],
    "Image Enhancer":               ["cloudinary", "huggingface", "segmind", "pixazo"],
    "Image Enhancer Plus":          ["cloudinary", "huggingface", "pixazo", "krea"],
    "HDR Master":                   ["huggingface", "segmind", "pixazo"],
    "HDR Booster":                  ["huggingface", "segmind", "pixazo"],
    "Noise Reducer":                ["huggingface", "cloudflare", "pixazo"],
    "Detail Enhancer":              ["huggingface", "krea", "pixazo"],
    "Image UpScaler":               ["huggingface", "cloudflare", "krea"],
    "AI 4K Image Upscaler":         ["huggingface", "cloudflare", "krea"],
    "Style Transfer":               ["huggingface", "pollinations", "together"],
    "Cartoonizer":                  ["huggingface", "pollinations", "deepai"],
    "Sketch Maker":                 ["huggingface", "pollinations"],
    "Vintage Maker":                ["huggingface", "pollinations", "deepai"],
    "Color Grader":                 ["cloudinary", "huggingface", "segmind"],
    "Color Corrector":              ["cloudinary", "huggingface"],
    "GFPGAN":                       ["huggingface", "deepai"],
    "CodeFormer":                   ["huggingface"],
    "Lighting Fixer":               ["huggingface", "segmind", "pixazo"],
    "Shadow Fixer":                 ["huggingface", "segmind", "pixazo"],
    "Exposure Fixer":               ["huggingface", "cloudinary", "pixazo"],
    "Sharpen Tool":                 ["huggingface", "pixazo"],
    "Blur Tool":                    ["huggingface", "cloudinary"],
    "Black & White":                ["huggingface", "cloudinary"],
    "Sepia Filter":                 ["huggingface", "cloudinary"],
    # ── Video tools ─────────────────────────────────────────────────
    "Slow-Mo Magic":                ["huggingface", "cloudflare"],
    "Fast-Forward Flash":           ["huggingface", "cloudflare"],
    "Cinematic Pulse":              ["pollinations", "together"],
    "VHS Nostalgia":                ["huggingface", "pollinations"],
    "Neon Pulse":                   ["huggingface", "pollinations"],
    "Glitch Pop":                   ["huggingface", "pollinations"],
    "Retro Reel":                   ["huggingface", "pollinations"],
    "Color Grade Pro":              ["cloudinary", "huggingface", "segmind"],
    "Motion Blur Trail":            ["huggingface", "cloudinary"],
    "Auto Caption Generator":       ["gemini", "groq", "mistral"],
    "Photo to Video Creator":       ["pollinations", "together"],
    "AI Motion Transfer Engine":    ["huggingface", "cloudflare"],
    "AI Cinematic Action Generator":["pollinations", "together"],
    "AI Consistent Motion Animator":["huggingface", "cloudflare"],
}
...
CAPABILITY_PROVIDERS: Dict[str, List[str]] = {
    ...
    "image-enhancement": ["cloudinary", "huggingface", "segmind", "pixazo"],
    "denoising":         ["huggingface", "cloudflare"],
    ...
    "audio-sync":        ["cloudflare"],
    "visualization":     ["pollinations", "gemini"],
    "image-gen":         ["pollinations", "together", "krea", "pixazo", "segmind"],
    "video-gen":         ["pollinations", "together"],
    "basic-processing":  ["huggingface", "pollinations", "cloudflare"],
    "controlnet":        ["segmind", "huggingface"],
}
...
class Pipeline4K:
    ...
    async def _post_process(self, img_bytes: bytes, file_mime: str) -> Optional[bytes]:
        ...
        try:
            ...
            ow, oh = img.size
            # Upscale to 4K only if image is smaller — preserve aspect ratio
...
_job_manager   = JobManager(
    ...
    require_redis=False,  # Graceful fallback to in-memory on Render free tier (no Redis addon)
)
...
app = FastAPI(
    title="Luminorbit",
    version="23.0.0",
    description="15-provider AI backend — TRUE 4K Studio — v23 Production",
    lifespan=lifespan,
...
if _RATE_LIMIT_AVAILABLE:
    ...
    logger.warning("[app] slowapi not installed — rate limiting disabled. Install: pip install slowapi")
